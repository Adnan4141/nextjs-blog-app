...
const TinyMCE = dynamic(() => import("@/components/RichTextEditor/TinyMCE"), {
  ssr: false,
});
...
const UpdatePostForm = ({ distinctCategories, postData }) => {
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    if (!formData.image) return setError("Please upload the image");
    ...
    try {
      const res = await axios.put(`/api/posts/${postData?._id}`, formData);
      if (res.data.success) {
  ...
  return (
    <div className="p-4 py-20   max-w-3xl mx-auto  text-gray-900 dark:text-white bg-white dark:bg-black">
      <h1 className="text-3xl font-bold mb-6 text-center">Update a Post</h1>
      ...
      <form onSubmit={handleFormSubmit} className="flex flex-col gap-6">
        ...
        <TinyMCE
          initialValue={
            postData?.content || "<p>Start writing your content here...</p>"
          }
          formData={formData}
        ...
        <button
          ...
          className="w-full py-3 cursor-pointer bg-purple-700 text-white font-semibold rounded hover:bg-purple-800 disabled:opacity-50"
        >
